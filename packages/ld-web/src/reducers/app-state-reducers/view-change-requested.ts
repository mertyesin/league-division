...
async function viewChangeRequested(appState: AppState, action: ViewChangeRequested): Promise<AppState> {
    const toViewName: string = action.toViewName;

    const appStateUrl: string = 'http://localhost:5000/app-state/' + toViewName;

    try {
        appState = await fetch (appStateUrl, {timeout: 5000}).then(r => r.json()) as any;
    }
    catch(err)  {
